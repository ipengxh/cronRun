            ...
            <div class="panel panel-default">
                ...
                <div class="panel-body">
                    <form action="{{ route('node:update', $node->id) }}" method="POST" class="form-horizontal" role="form">
                        ...
                        <div class="form-group form-group-sm">
                            <label class="control-label col-sm-2">Bin paths:</label>
                            @foreach ($thisNodeBinPaths as $binPath)
                            <div class="row">
                                <div class="col-sm-2">
                                    <input type="text" class="form-control" name="bin_path[0][name]" value="{{ $binPath->name }}">
                                </div>
                                <div class="col-sm-6">
                                    <input type="text" class="form-control" name="bin_path[0][path]" value="{{ $binPath->path }}">
                                </div>
                            </div>
                            @endforeach
                        </div>
